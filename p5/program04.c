/*
* Програма 4. Використання АЦП та таймеру 1.
*
* Призначення програми: виводить на LCD дисплей 
* значення напруги. Переривання таймеру 1 використовується
* для оновлення LCD дисплея і виводу значення напруги.
*
* Обладнання:
...
#define DIGIT_COUNT        4                           /* Кількість цифр в числі, яке виводиться 
                                                          на LCD. */
#define NUMBER_MAX         9999                        /* Максимальне число, яке відображається. */
#define STRING_COUNT       DIGIT_COUNT+1               /* Довжина мосиву цифр числа. На одиницю
                                                          більше тому, що строка закінується 
														  символом '\0'. */
#define NUMBER_CODE        0x30                        /* Код ноля в таблиці ASCII. */

uint16_t count = 0;                                    /* Змінна для збереження поточного числа,
                                                          яке виводиться на LCD. */
...
uint8_t* num_to_string(uint16_t number)
{
	...
	number %= 1000;
    string[1] = number/100+NUMBER_CODE;
	number %= 100;
	string[2] = number/10+NUMBER_CODE;
	number %= 10;
    string[3] = number/1+NUMBER_CODE;	 
	string[4] = '\0';
...
{
	set_position(0, 1); 
	print_lcd(num_to_string(count));
	count++;
	if(count > NUMBER_MAX)                             /* Число містить більше цифр, чим задано?. */
	{
		count = 0;                                     /* Переведення в нуль. */
	}
}
...
int main(void)
{
    ...
    while (1)
    {
		...
		/* TODO: додати перетворення дійсного числа в строку та запис
           в масив відображення на LCD.
		   Та інше) */
		sei();
    }
}
