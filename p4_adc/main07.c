/*
* Програма 7. Перетворення коду АЦП в значення напруги.
*
* Призначення програми: виводить на дисплей значення
* напруги на вході АЦП.
*
* Обладнання:
...
#define DELAY_DISPLAY    19                            /* Кількість циклів повтору виведених цифр 
                                                          на дисплей. */
#define DIGIT_MIN        00                            /* Номер першого індикатору. */
#define DIGIT_MAX        03                            /* Номер останнього індикатору. */
...
/* Лічильники циклів. */
static uint8_t i = DIGIT_MAX;                          /* Поточний номер індикатору, починаючи 
                                                          із останнього. */
...
ISR(TIMER1_COMPA_vect)
{
    ...
    /* На відміну від попередньої програми індикатори
	   вмикаються від останнього до першого, тому що 
	   перший індикатор містить десяткову точку. */
	if(i > DIGIT_MIN)                                  /* Доки поточний номер індикатору не досяг 
	                                                      першого. */
	{
		i--;                                           /* Переходимо до попереднього індикатору. */
	}
	else                                               /* Поточний номер досяг номеру 
	                                                      першого індикатору. */
	{
		i = DIGIT_MAX;                                 /* Переходимо до останнього індикатору. */
		PORTD &= ~0x80;                                /* Увімкнення десяткової точки. */
...
int main(void)
{
	...
	while (1)
	{
		_delay_ms(100);                                /* Затримка між перетвореннями АЦП. */ 
