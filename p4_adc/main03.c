/*
* Програма 3. Робота з чотирьох цифровим семисегментним дисплеєм.
*
...
#define NUMBER_COUNT     07                            /* Кількість перших цифр чотирьох значних 
                                                          чисел: 0123, 1234, 2345, ..., 6789. */
...
int main03(void)
{
	...
	while (1)
	{
		for(j = 0; j < NUMBER_COUNT; j++)              /* Цикл виводу цифр на дисплей. */
		{	
			for(i = 0; i < DELAY_DISPLAY; i++)         /* Цикл повтору виведених цифр. */
			{
				...
				PORTB = DIGIT_2;
				PORTD = number_code[j+1];
				_delay_ms(DELAY_DIGIT);
				PORTB = DIGIT_3;
				PORTD = number_code[j+2];
				_delay_ms(DELAY_DIGIT);
				PORTB = DIGIT_4;
				PORTD = number_code[j+3];
				_delay_ms(DELAY_DIGIT);
			}
		}
	}
